...
export default function App() {
  ...
  useEffect(() => {
    const stored = window._motResponses || [];
    setStoredResponses(stored);
    setResponses(stored);
  }, []);

  const saveResp = (r) => {
    const up = [...storedResponses, r];
    window._motResponses = up;
    setStoredResponses(up);
    setResponses(up);
  };
  ...
  const handleSubmit = async () => {
    ...
    saveResp({ name: rName, email: rEmail, timestamp: new Date().toISOString(), products, purchaseRating, purchaseComment, firstImpression, usageFreq, skinConcerns: skinOther ? [...skinConcerns, `Other: ${skinOther}`] : skinConcerns, resultsLevel, resultsDesc, timeToResults, satisfaction, recommend, recommendWhy, suggestions });
    setSubmitting(false);
    setView("thanks");
  };
  ...
  const resetForm = () => {
    ...
    setSatisfaction(0); setRecommend(""); setRecommendWhy(""); setSuggestions(""); setRName(""); setREmail(""); setView("form");
  };

  const getStats = () => {
    const rr = storedResponses;
    if (!rr.length) return null;
    const n = rr.length;
    const avgSat = rr.reduce((a, r) => a + r.satisfaction, 0) / n;
    const purchaseDist = ["Excellent","Good","Average","Poor"].map(l => ({ label: l, count: rr.filter(r => r.purchaseRating === l).length }));
    const resultsDist = ["Yes, significant improvement","Yes, some improvement","Too early to tell","No visible change yet"].map(l => ({ label: l.length > 24 ? l.slice(0,24)+"…" : l, full: l, count: rr.filter(r => r.resultsLevel === l).length }));
    const recommendDist = ["Absolutely","Probably","Not sure","Unlikely"].map(l => ({ label: l, count: rr.filter(r => r.recommend === l).length }));
    const productDist = PRODUCTS.map(p => ({ label: p, count: rr.filter(r => r.products.includes(p)).length }));
    const concernDist = SKIN_CONCERNS.map(c => ({ label: c.split("/")[0].trim(), count: rr.filter(r => r.skinConcerns.includes(c)).length }));
    const promoters = rr.filter(r => r.recommend === "Absolutely" || r.recommend === "Probably").length;
  ...
  const downloadReport = () => {
    ...
    const txt = [
      ...
      "", "ALL RESPONSES", "-------------",
      ...rr.map((r,i) => [`#${i+1} | ${r.name||"Anonymous"} | ${new Date(r.timestamp).toLocaleString("en-IN")}`, `  Products: ${r.products.join(", ")}`, `  Purchase: ${r.purchaseRating} | Satisfaction: ${r.satisfaction}/10 | Recommend: ${r.recommend}`, `  Usage: ${r.usageFreq} | Time to results: ${r.timeToResults}`, `  Skin concerns: ${r.skinConcerns.join(", ")}`, `  Results: ${r.resultsLevel}`, r.resultsDesc?`  Detail: ${r.resultsDesc}`:"", r.firstImpression?`  First impression: ${r.firstImpression}`:"", r.suggestions?`  Suggestions: ${r.suggestions}`:"", ""].filter(Boolean).join("\n"))
    ].join("\n");
    const a = document.createElement("a");
    a.href = URL.createObjectURL(new Blob([txt], {type:"text/plain"}));
    a.download = `MoT-Feedback-${new Date().toISOString().slice(0,10)}.txt`; a.click();
  };
    ...
    <div style={{ minHeight: "100vh", background: C.bg }}>
      ...
      <div style={{ maxWidth: "680px", margin: "0 auto", padding: "36px 20px 60px" }}>
        ...
        <Card>
          ...
          {resultsLevel.startsWith("Yes") && (<><Div /><label style={{ display: "block", fontSize: "12px", color: C.muted, marginBottom: "8px", textTransform: "uppercase", letterSpacing: "0.8px" }}>Please describe what you noticed:</label><TA value={resultsDesc} onChange={setResultsDesc} placeholder="e.g., My dark spots faded noticeably, skin felt more hydrated and bright…" /></>)}
        </Card>
        <Card>
        ...
        <Card>
          ...
          {satisfaction > 0 && <p style={{ margin: "12px 0 0", fontSize: "14px", color: satisfaction>=8?C.success:satisfaction>=5?C.gold:"#C0392B", fontFamily: "'Cormorant Garamond',serif", fontWeight: "600" }}>
        ...
        <Card style={{ background: "#FFF8EC" }}>
          <p style={{ margin: "0 0 14px", fontFamily: "'Cormorant Garamond',serif", fontSize: "17px", color: C.brown, fontWeight: "700" }}>Your Details <span style={{ color: C.muted, fontSize: "13px", fontWeight: "400" }}>(optional)</span></p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
            <div><label style={{ display: "block", fontSize: "11px", color: C.muted, marginBottom: "6px", textTransform: "uppercase", letterSpacing: "0.8px" }}>Name</label><TI value={rName} onChange={setRName} placeholder="Your name" /></div>
            <div><label style={{ display: "block", fontSize: "11px", color: C.muted, marginBottom: "6px", textTransform: "uppercase", letterSpacing: "0.8px" }}>Email</label><TI value={rEmail} onChange={setREmail} placeholder="your@email.com" /></div>
          </div>
        </Card>
      ...
      {!adminUnlocked ? (
        ...
        <div style={{ maxWidth: "860px", margin: "0 auto", padding: "32px 20px 60px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "16px", marginBottom: "24px" }}>
            {[{ label: "Total Responses", val: stats.n }, { label: "Avg Satisfaction", val: `${stats.avgSat.toFixed(1)}/10` }, { label: "Recommendation Rate", val: `${stats.nps}%` }].map((s,i) => (
          ...
          <Card>
            ...
            {[...storedResponses].reverse().slice(0,5).map((r,i) => (
